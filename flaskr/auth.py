...
def register():
    if request.method == "POST":
        username = request.form["username"]  # does this username link to the db column name?
        password = request.form["password"]
        db = get_db()  # why we need to get the db here?
        error = None
...
# for that.
# bp.before_app_request() links/register a function that runs before other view
...
# editting or deleting blog posts.
# Note that we use @functools.wraps() here to keep the same
# metadata of the original function preserved.
# Later on, we will use this decorator to decorate other views
# such as create, edit, delete blog posts.
